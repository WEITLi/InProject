...
def from_pc(act, ul):
    ...
    if act_pc == user_pc:
        ...
        return (1, act_pc)
    elif (ul.loc[act['user']]['sup'] is not None and 
          ul.loc[act['user']]['sup'] in ul.index and 
          act_pc == ul.loc[ul.loc[act['user']]['sup']]['pc']):
        return (3, act_pc)
    else:
        return (2, act_pc)
...
def process_week_num(week, users, userlist = 'all', data = 'r4.2', config_id = None):
    ...
    # 过滤活动数据，只保留来自筛选用户的活动，避免KeyError
    print(f"周 {week}: 过滤前活动数量: {len(acts_week)}")
    acts_week = acts_week[acts_week['user'].isin(userlist)]
    print(f"周 {week}: 过滤后活动数量: {len(acts_week)} (用户数: {len(userlist)})")
    
    if len(acts_week) == 0:
        print(f"警告: 周 {week} 在用户过滤后没有活动数据")
        # 创建空的DataFrame保存
        empty_df = pd.DataFrame(columns=['actid','pcid','time_stamp'] + ['user','day','act','pc','time'] + 
                               ['usb_dur'] + (['file_tree_len'] if data not in ['r4.2','r4.1'] else []) +
                               ['file_type', 'file_len', 'file_nwords', 'disk', 'file_depth'] + 
