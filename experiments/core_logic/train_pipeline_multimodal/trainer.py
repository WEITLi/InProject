...
class Trainer:
    ...
    def train_epoch(self, epoch_num: int) -> float:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            print(f"[DEBUG Trainer] Batch {batch_idx}: inputs keys: {list(inputs.keys())}")
            for key, value in inputs.items():
                if isinstance(value, torch.Tensor):
                    print(f"[DEBUG Trainer] Batch {batch_idx}: {key} shape: {value.shape}")
                elif isinstance(value, list):
                    print(f"[DEBUG Trainer] Batch {batch_idx}: {key} type: list, length: {len(value)}")
                else:
                    print(f"[DEBUG Trainer] Batch {batch_idx}: {key} type: {type(value)}")
            print(f"[DEBUG Trainer] Batch {batch_idx}: labels shape: {labels.shape}")
            
            self.optimizer.zero_grad()
            
            # Forward pass
            print(f"[DEBUG Trainer] Batch {batch_idx}: About to call model.forward()")
            try:
                outputs = self.model(inputs) # model's forward should return a dict with 'logits'
                print(f"[DEBUG Trainer] Batch {batch_idx}: model.forward() completed successfully")
                print(f"[DEBUG Trainer] Batch {batch_idx}: outputs keys: {list(outputs.keys())}")
                logits = outputs['logits'] 
                print(f"[DEBUG Trainer] Batch {batch_idx}: logits shape: {logits.shape}")
            except Exception as e:
                print(f"[DEBUG Trainer] Batch {batch_idx}: ERROR in model.forward(): {e}")
                print(f"[DEBUG Trainer] Batch {batch_idx}: Exception type: {type(e)}")
                import traceback
                print(f"[DEBUG Trainer] Batch {batch_idx}: Traceback:")
                traceback.print_exc()
                raise e
    ...
    def evaluate(self, epoch_num: int, loader: DataLoader, phase: str = "Val") -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(progress_bar):
                ...
                print(f"[DEBUG Trainer {phase}] Batch {batch_idx}: About to call model.forward()")
                try:
                    outputs = self.model(inputs)
                    print(f"[DEBUG Trainer {phase}] Batch {batch_idx}: model.forward() completed successfully")
                    logits = outputs['logits']
                    ...
                    print(f"[DEBUG Trainer {phase}] Batch {batch_idx}: ERROR in model.forward(): {e}")
                    print(f"[DEBUG Trainer {phase}] Batch {batch_idx}: Exception type: {type(e)}")
                    import traceback
                    print(f"[DEBUG Trainer {phase}] Batch {batch_idx}: Traceback:")
                    traceback.print_exc()
                    raise e
